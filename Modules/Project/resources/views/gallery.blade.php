...
@section('title')
    <title>{{ __('translate.Project Gallery') }}</title>
@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('translate.Project Gallery') }}</h3>
    <p class="crancy-header__text">{{ __('translate.Manage Service') }} >> {{ __('translate.Project Gallery') }}</p>
@endsection
                            ...
                            <div class="col-12 mg-top-30">
                                ...
                                <div class="crancy-product-card">
                                    <div class="create_new_btn_inline_box">
                                        <h4 class="crancy-product-card__title">{{ __('translate.Project') }} : {{ html_decode($project->translate->title) }} <span data-toggle="tooltip" data-placement="top" class="fa fa-info-circle text--primary" title="{{ __('translate.Max 3 Files') }}"></span>
                                        </h4>
                                    </div>

                                    <div class="row mg-top-30">

                                        <form id="dropzoneForm" method="post" action="{{ route('admin.upload-project', $project->id) }}"  enctype="multipart/form-data"  class="dropzone">
                                        @csrf
...
@push('js_section')
    ...
    <script>
        ...
        function itemDeleteConfrimation(id){
            $("#item_delect_confirmation").attr("action",'{{ url(config('admin.prefix').'/delete-project-gallery/') }}'+"/"+id)
        }
    </script>
